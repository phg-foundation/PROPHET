...
temp_data <- data.frame(primary_lifestyle_factors_type = c("Air Pollution"))
...
fin_dataframe_sub <- fin_dataframe %>% filter(!match_column %in% plot_data$match_column)
plot_data <- rbind(plot_data, fin_dataframe_sub) %>% dplyr::select(-match_column) %>% dcast(., population_type ~ cvd_type, value.var = "Freq") %>% column_to_rownames("population_type")
...
table_n_col(data = plot_data, null_column_name =  null_columns,  should_group = F, table_width = 1700, num_colors = 5, 
            table_name = "Population vs \n CVDs", output_name = "population_disease_primary")
##might not always print correctly - please do control+P of the html page produced to save as pdf
...
table_n_col(data = plot_data, null_column_name =  null_columns,  should_group = F, table_width = 1700, num_colors = 5, 
            table_name = "Population vs \n CVDs", output_name = "population_disease_secondary")
```
